	...
	this.J_f = jQuery.ajax({
		...
		success: function(D, C, _) {
			...
			try {
				...
				F = [];
				if (mini_debugger == true) mini.showMsg(D);//alert("tree json is error.")
			}
			var G = {
				result: F,
				data: F,
...
_1751 = function(C) {
	...
	mini[PIm](C, I, ["id", "name", "width", "height", "borderStyle", "value", "defaultValue", "contextMenu", "tooltip", "ondestroy", "diyAttribute"]);
	...
	this.J_f = jQuery.ajax({
		...
		success: function(G, E, D) {
			...
			try {
				...
				if (mini_debugger == true) mini.showMsg(G); //alert("datagrid json is error.")
			}
			if (K == null) K = {
				data: [],
				total: 0
	...
	this.J_f = jQuery.ajax({
		...
		success: function($) {
			...
			try {
				...
				_ = [];
				if (mini_debugger == true) mini.showMsg($); //alert("json is error.")
			}
			var A = {
				data: _,
				cancel: false
...
//�����Զ������Եķ���
WK5N.prototype.getDiyAttribute = function(){
	return this.diyAttribute;
};
WK5N.prototype.setDiyAttribute = function(diyAttribute){
	this.diyAttribute = diyAttribute;
};
...
mini.I8Y1 = function(N) {
	...
	for (var M = 0, H = O.length; M < H; M++) {
		...
		mini[PIm](C, D, ["name", "header", "field", "editor", "filter", "renderer", "width", "type", "renderer", "headerAlign", "align", "headerCls", "cellCls", "headerStyle", "cellStyle", "displayField", "dateFormat", "listFormat", "mapFormat", "trueValue", "falseValue", "dataType", "vtype", "diyAttribute"]);
		...
		doEncode = function($) {
			...
			if (A == "undefined") {
				...
				for (_ in $){
					var flag = true;
					try{
						flag = !useHasOwn || $.hasOwnProperty(_);
					}catch(e){flag = true;}
					if (flag) {
						E = $[_];
						A = typeof E;
