...
<%!/*加载行政区划数据*/
	...
	//资源总量
	public void statResCount(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		int pageIndex = Integer.parseInt(request.getParameter("pageIndex"));
		int pageSize = Integer.parseInt(request.getParameter("pageSize"));
		String submitJSON = request.getParameter("submitData");
		//System.out.println("*********submit:" + submitJSON);
		if(submitJSON == null || "".equals(submitJSON.trim())){
			submitJSON = (String)request.getSession().getAttribute("submitData");
		}else{
			request.getSession().setAttribute("submitData", submitJSON);
		}

		ResourceStatImpl impl = new ResourceStatImpl();
		String jsonData = impl.statResCount(submitJSON, pageIndex, pageSize);
		response.getWriter().write(jsonData);
	}
	
	//终端活跃度
	public void statResClickCount(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		int pageIndex = Integer.parseInt(request.getParameter("pageIndex"));
		int pageSize = Integer.parseInt(request.getParameter("pageSize"));
		String submitJSON = request.getParameter("submitData");
		...
		ResourceStatImpl impl = new ResourceStatImpl();
		String jsonData = impl.statResActivity(submitJSON, pageIndex, pageSize);
		response.getWriter().write(jsonData);
	}
	
	//热门资源
	public void statHotRes(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		int pageIndex = Integer.parseInt(request.getParameter("pageIndex"));
		int pageSize = Integer.parseInt(request.getParameter("pageSize"));
		String submitJSON = request.getParameter("submitData");
		...
		ResourceStatImpl impl = new ResourceStatImpl();
		String jsonData = impl.statHotRes(submitJSON, pageIndex, pageSize);
		response.getWriter().write(jsonData);
	}
