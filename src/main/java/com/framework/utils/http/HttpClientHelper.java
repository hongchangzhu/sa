...
public final class HttpClientHelper {
	...
	private static int MAX_TOTEL = 1000;
	private static int MAX_CONNECTION_PER_ROUTE = 100;
	...
	/**
	 * 该方法主要是为了解决上面的方便只能够处理200状态其他全部返回null的局限 该方法可以返回状态吗以及服务器的响应信息，根据状态等信息自己再处理
	 * 
	 * @param request
	 *            请求参数
	 * 
	...
	public HashMap<String, Object> executeAndReturnMap(HttpRequestBase request, String charset) {
		...
		try {
			...
			if (resp.getStatusLine().getStatusCode() == 200) {
				...
				returnMap.put("message", formatByte(EntityUtils.toByteArray(entity), charset));
				logger.error("{}\nreturn error httpstatus code:{}", request.getURI().toString(), resp.getStatusLine()
						.getStatusCode());
			}
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
